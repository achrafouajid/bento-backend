...
#
# bento-backend is a PUBLIC repo, so the pull needs no credentials. Force the
# request anonymous: GIT_TERMINAL_PROMPT=0 fails fast instead of hanging on a
# username prompt, and clearing credential.helper / the cached github.com
# extraheader stops a stale token on the box from turning an anonymous 200 into
# a 401. Do NOT reuse this pattern if either repo is ever made private.
git_pub() {
