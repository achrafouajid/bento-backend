...
public interface PaymentRepository extends JpaRepository<Payment, UUID> {
    ...
    @Query("SELECT p FROM Payment p WHERE p.organizationId = :orgId ORDER BY p.paymentDate DESC")
    ...
    @Query("SELECT p FROM Payment p WHERE p.organizationId = :orgId AND p.partnerId = :partnerId ORDER BY p.paymentDate")
    List<Payment> findByOrganizationIdAndPartnerId(@Param("orgId") UUID orgId, @Param("partnerId") UUID partnerId);

    @Query("SELECT p FROM Payment p WHERE p.organizationId = :orgId AND p.invoiceId = :invoiceId ORDER BY p.paymentDate")
    ...
    @Query("""
            ...
            WHERE p.organizationId = :orgId AND p.partnerId = :partnerId AND p.invoiceId IS NOT NULL
